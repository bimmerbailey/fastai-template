...
async def test_chat_returns_response(
    chat_client: AsyncClient, sample_user_id: uuid.UUID
) -> None:
    """POST /chat returns a valid ChatResponse with conversation_id."""
    res = await chat_client.post(BASE_URL, json=_chat_payload(sample_user_id))
...
async def test_chat_missing_message_rejected(
    ...
    res = await chat_client.post(BASE_URL, json={"user_id": str(sample_user_id)})
    assert res.status_code == 422
...
async def test_chat_creates_new_conversation(
    ...
    conv_res = await chat_client.get(f"{CONVERSATIONS_URL}/{conversation_id}")
...
async def test_chat_persists_user_and_assistant_messages(
    ...
    msgs_res = await chat_client.get(f"{CONVERSATIONS_URL}/{conversation_id}/messages")
